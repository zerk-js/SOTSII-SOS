...
// Type: Kerberos.Sots.GameStates.DesignScreenState
		...
		private static Dictionary<string, DesignScreenState.BankFilter> BankFilters = new Dictionary<string, DesignScreenState.BankFilter>()
	{
	  ...
	  },
	  {
		...
		protected override void OnPrepare(GameState prev, object[] stateParams)
		{
			...
			foreach (ShipSectionAsset section in this._showDebugControls ? (IEnumerable<ShipSectionAsset>)this.App.AssetDatabase.ShipSections : this.App.Game.GetAvailableShipSections(this.App.LocalPlayer.ID).Where<ShipSectionAsset>((Func<ShipSectionAsset, bool>)(x => DesignScreenState.DesignScreenAllowsShipClass(x))))
		...
		private void CollectWeapons(ShipSectionAsset section)
		{
			...
			for (int sectionModuleMountIndex = 0; sectionModuleMountIndex < section.Modules.Length; ++sectionModuleMountIndex)
			{
				...
				foreach (LogicalModule logicalModule in (IEnumerable<LogicalModule>)LogicalModule.EnumerateModuleFits(this.App.GameDatabase.GetAvailableModules(this.App.AssetDatabase, this.App.LocalPlayer.ID), section, sectionModuleMountIndex, this._showDebugControls).ToList<LogicalModule>())
				{
		...
		private void RepopulateDebugControls()
		{
			if (!this._showDebugControls)
				return;
			this.App.UI.ClearItems(DesignScreenState.DebugEmpireColorList);
			for (int index = 0; index < Player.DefaultPrimaryPlayerColors.Count; ++index)
			{
				...
				this.App.UI.Send((object)"AddItem", (object)DesignScreenState.DebugEmpireColorList, (object)index.ToString(), (object)"colorbox");
				this.App.UI.Send((object)"SetItemColor", (object)DesignScreenState.DebugEmpireColorList, (object)index.ToString(), (object)primaryPlayerColor.X, (object)primaryPlayerColor.Y, (object)primaryPlayerColor.Z);
			}
			this.App.UI.ClearItems(DesignScreenState.DebugAnimTracks);
			for (int userItemId = 0; userItemId < DesignScreenState._debugAnimTracks.Length; ++userItemId)
				this.App.UI.AddItem(DesignScreenState.DebugAnimTracks, string.Empty, userItemId, DesignScreenState._debugAnimTracks[userItemId]);
			this.SetSelectedAnimTrack(DesignScreenState._debugAnimTracks[0], string.Empty);
			this.App.UI.ClearItems(DesignScreenState.DebugAnimMode);
			for (int userItemId = 0; userItemId < DesignScreenState._debugAnimMode.Length; ++userItemId)
				this.App.UI.AddItem(DesignScreenState.DebugAnimMode, string.Empty, userItemId, DesignScreenState._debugAnimMode[userItemId]);
			this.App.UI.SetSelection(DesignScreenState.DebugAnimMode, 0);
		}
		...
		private IEnumerable<int> CollectWeaponDesigns(
		  ...
		  WeaponEnums.TurretClasses turretClass)
		{
			IEnumerable<DesignInfo> designs = this.App.GameDatabase.GetDesignInfosForPlayer(this.App.LocalPlayer.ID);
		  ...
		  LogicalWeapon includeSelectedWeapon)
		{
			if (WeaponEnums.IsBattleRider(turretClass))
			{
				...
				if (logicalWeapon != null && !section.IsWraithAbductor)
				{
					...
					{
						IEnumerable<LogicalWeapon> collection = LogicalWeapon.EnumerateWeaponFits(section.Faction, section.SectionName, this.App.GameDatabase.GetAvailableWeapons(this.App.AssetDatabase, this.App.LocalPlayer.ID).Where<LogicalWeapon>((Func<LogicalWeapon, bool>)(weapon => weapon.IsVisible)), turretSize, turretClass);
		...
		protected override void OnEnter()
		{
			...
			this.App.UI.SetPropertyBool(DesignScreenState.UISectionList(ShipSectionType.Command), "only_user_events", true);
			this.App.UI.SetPropertyBool(DesignScreenState.UISectionList(ShipSectionType.Engine), "only_user_events", true);
			this.App.UI.SetPropertyBool(DesignScreenState.UISectionList(ShipSectionType.Mission), "only_user_events", true);
			this._RetrofitMode = false;
			EmpireBarUI.SyncTitleFrame(this.App);
			this.PopulateFactionsList();
			this.SetSelectedFaction(this.App.GameDatabase.GetFactionName(this.App.GameDatabase.GetPlayerFactionID(this.App.LocalPlayer.ID)), "init");
		...
		protected override void OnExit(GameState prev, ExitReason reason)
		{
			...
			this._combatShipQueue = (DesignScreenState.CombatShipQueue)null;
		...
		public DesignScreenState(App game)
		  : base(game)
		{
		}
		...
		private void UICommChannel_GameEvent(string eventName, string[] eventParams)
		{
			if (!(eventName == "IconClicked") || eventParams.Length != 1)
				return;
			string[] strArray = eventParams[0].Split(':');
			string str = strArray[0];
			if (str == "WeaponGroup")
			{
				IWeaponShipData currentWeaponBank = this._selection.GetCurrentWeaponBank(this.App.GetGameObject<WeaponBank>(int.Parse(strArray[1])));
				this.HideModuleSelector();
			...
			{
				ModuleShipData currentModuleMount = this._selection.GetCurrentModuleMount(this.App.GetGameObject<Kerberos.Sots.GameObjects.Section>(int.Parse(strArray[1])).ShipSectionAsset, strArray[2]);
				this.HideWeaponSelector();
			...
			else
			{
				if (!(str == "Psionic"))
					return;
				Module gameObject = this.App.GetGameObject<Module>(int.Parse(strArray[1]));
				int psiindex = int.Parse(strArray[2]);
				ModuleShipData currentModuleMount = this._selection.GetCurrentModuleMount(gameObject.AttachedSection.ShipSectionAsset, gameObject._attachment.NodeName);
				if (this._selectedModule == currentModuleMount)
				{
					this.HidePsionicSelector();
				}
				...
				{
					this._selectedModule = currentModuleMount;
					this._PsionicIndex = psiindex;
					this.ShowPsionicSelector(gameObject, psiindex);
				}
			}
		}
		...
		private void ShowModuleSelector(ModuleShipData moduleData)
		{
			...
			if (moduleData != null)
			{
				...
				if (this._RetrofitMode && this._selectedModule.SelectedModule != null && ((IEnumerable<LogicalBank>)this._selectedModule.SelectedModule.Module.Banks).Any<LogicalBank>((Func<LogicalBank, bool>)(x => WeaponEnums.IsBattleRider(x.TurretClass))))
					return;
		...
		private void PopulateModuleSelector(List<ModuleData> modules)
		{
			...
			this._moduleSelector.SetAvailableModules(this._RetrofitMode ? modules.Where<ModuleData>((Func<ModuleData, bool>)(x =>
		   {
			   if (((IEnumerable<LogicalBank>)x.Module.Banks).Any<LogicalBank>())
				   return ((IEnumerable<LogicalBank>)x.Module.Banks).Any<LogicalBank>((Func<LogicalBank, bool>)(j => !WeaponEnums.IsBattleRider(j.TurretClass)));
			   return true;
		   })).Select<ModuleData, LogicalModule>((Func<ModuleData, LogicalModule>)(x => x.Module)) : modules.Select<ModuleData, LogicalModule>((Func<ModuleData, LogicalModule>)(x => x.Module)), this._selectedModule.SelectedModule != null ? this._selectedModule.SelectedModule.Module : (LogicalModule)null, true);
		  ...
		  LogicalPsionic selected)
		{
			...
			this._psionicSelector.SetAvailablePsionics(psionics.Where<LogicalPsionic>((Func<LogicalPsionic, bool>)(x =>
		   {
			   if (this.PsiNotSelected(x))
				   return x.IsAvailable(this.App.GameDatabase, this.App.LocalPlayer.ID, false);
			   return false;
		   })), selected, true);
		...
		protected void SelectFilter(string panel, bool enabled)
		{
			...
			if (enabled)
			{
				if (panel == "filterAll")
				{
					foreach (KeyValuePair<string, DesignScreenState.BankFilter> bankFilter in DesignScreenState.BankFilters)
					{
			...
			if (!DesignScreenState.BankFilters.Where<KeyValuePair<string, DesignScreenState.BankFilter>>((Func<KeyValuePair<string, DesignScreenState.BankFilter>, bool>)(x => x.Value.Enabled)).Any<KeyValuePair<string, DesignScreenState.BankFilter>>())
			{
		  ...
		  string[] msgParams)
		{
			if (msgType == "firing_mode")
			{
				...
				if (gameObject != null)
					this.SetWeaponsAllFireMode(gameObject.Weapon, fireMode);
			}
			else if (msgType == "filter_mode")
			{
				this.HideModuleSelector();
				this.HidePsionicSelector();
				this.HideWeaponSelector();
			...
			{
				if (panelName == this._deleteItemDialog && bool.Parse(msgParams[0]))
				{
			...
			else if (msgType == "checkbox_clicked")
			{
				...
				else if (panelName.Contains("tech"))
				{
					this._RetrofitMode = false;
				...
				else if ("debugAutoAssignModules" == panelName)
				{
					ModuleShipData.DebugAutoAssignModules = int.Parse(msgParams[0]) != 0;
					this.CurrentShipChanged();
				}
			}
			else if (msgType == "button_clicked")
			{
				if (panelName == "designRemove")
				{
					...
					{
						this.App.GameDatabase.RemovePlayerDesign(this._selectedDesign.Value);
						this.PopulateDesignList();
					}
				}
				else if (panelName.Contains("designDeleteButton"))
				{
					this._deleteItemID = int.Parse(panelName.Split('|')[1]);
				...
				else if (panelName == "expandFilters")
				{
					DesignScreenState.FilterVisible = !DesignScreenState.FilterVisible;
					this.App.UI.SetVisible("BankFilter", DesignScreenState.FilterVisible);
				}
				else if (panelName == "gameTutorialButton")
				{
					...
					this.HidePsionicSelector();
					this.HideWeaponSelector();
					this.App.UI.SetVisible("DesignScreenTutorial", true);
				}
				else if (panelName == "designScreenTutImage")
				{
					this.HideModuleSelector();
					this.HidePsionicSelector();
					this.HideWeaponSelector();
					this.App.UI.SetVisible("DesignScreenTutorial", false);
				}
				...
				else if (panelName == "gameCommitButton")
					this.ShowCommitDialog();
				else if (panelName == "submit_dialog_ok")
				{
					this.App.SteamHelper.DoAchievement(AchievementType.SOTS2_SHIPYARD_SUPERVISOR);
					this.CommitDesign();
					this.App.UI.SetVisible("DesignUpdate1", true);
					this.App.UI.SetVisible("DesignUpdate2", true);
					this.App.UI.SetVisible("DesignUpdate3", true);
					this.App.UI.SetVisible("DesignUpdate4", true);
				}
				else if (panelName == "submit_dialog_cancel")
					this.App.UI.SetVisible("submit_dialog", false);
				else if ("starmap" == panelName)
					this.App.SwitchGameState<StarMapState>();
				else if ("combatAddCurrentDesign" == panelName)
					this.GuaranteedCombatShipQueue.AddDesign(this.App, this._builder, 1, this.App.LocalPlayer.ID);
				else if ("combatAddCurrentDesignX10" == panelName)
					this.GuaranteedCombatShipQueue.AddDesign(this.App, this._builder, 10, this.App.LocalPlayer.ID);
				else if ("combatAddCurrentDesignAsAI" == panelName)
					this.GuaranteedCombatShipQueue.AddDesign(this.App, this._builder, 1, this.App.Game.OtherPlayers[0].ID);
				else if (!("combatStartRandom" == panelName))
				{
					if ("combatStart" == panelName)
						this.GuaranteedCombatShipQueue.Start(this.App, null, 0);
					else if ("combatStartNearStar" == panelName)
						this.GuaranteedCombatShipQueue.Start(this.App, "data\\CombatNearStar.xml", 0);
					else if ("combatStartNearBigStar" == panelName)
						this.GuaranteedCombatShipQueue.Start(this.App, "data\\CombatNearGiantStar.xml", 0);
					else if ("combatStartNearEarth" == panelName)
						this.GuaranteedCombatShipQueue.Start(this.App, "data\\CombatNearEarth.xml", 0);
					else if ("combatStartNearSaturn" == panelName)
						this.GuaranteedCombatShipQueue.Start(this.App, "data\\CombatNearSaturn.xml", 0);
					else if ("combatNuPlanets" == panelName)
						this.GuaranteedCombatShipQueue.Start(this.App, "data\\CombatNuPlanets.xml", 0);
					else if ("combatStartSystem" == panelName)
						this.GuaranteedCombatShipQueue.Start(this.App, "data\\CombatRandomSystem.xml", 0);
					else if ("combatStartScratch" == panelName)
						this.GuaranteedCombatShipQueue.Start(this.App, "data/scratch_combat.xml", 0);
					else if ("weaponTest" == panelName)
					{
						...
						this.App.UI.SetVisible("debugStuff", this._showDebugControls);
					...
					else if (panelName == DesignScreenState.DebugPlayAnim)
						this.PlayAnim();
					else if (panelName == DesignScreenState.DebugStopAnim)
						this.StopAnim();
					else if (panelName == "gameRetrofitButton" && this._selectedDesign.HasValue)
					{
						DesignInfo designInfo = this.App.GameDatabase.GetDesignInfo(this._selectedDesign.Value);
						if (this.App.GameDatabase.GetPlayerTechInfos(this.App.LocalPlayer.ID).Any<PlayerTechInfo>((Func<PlayerTechInfo, bool>)(x =>
					   {
						   if (x.State == TechStates.Researched)
							   return x.TechFileID == "ENG_Orbital_Drydocks";
						   return false;
					   })) && designInfo.isPrototyped && designInfo.DesignDate != this.App.GameDatabase.GetTurnCount())
						{
			...
			else if (msgType == "list_sel_changed")
			{
				if (panelName == "gameWeaponDesignList")
				{
					...
					{
						int design = weaponSelectorBank.Designs[index];
						if (design != weaponSelectorBank.SelectedDesign)
						{
							weaponSelectorBank.SelectedDesign = design;
							weaponSelectorBank.SelectedWeapon = (LogicalWeapon)null;
							if (!this._RetrofitMode)
							{
								this._selectedDesign = new int?();
								this._selectedDesignPW = "";
				...
				else if (panelName == "gameModuleList")
				{
					ModuleShipData selectedModule = this._selectedModule;
					if (selectedModule != null)
					{
						ModuleData moduleData = (ModuleData)null;
						if (!string.IsNullOrEmpty(msgParams[0]))
						{
							int index = int.Parse(msgParams[0]);
							if (index >= 0)
								moduleData = selectedModule.Modules[index];
						}
				...
				else if (panelName == DesignScreenState.DebugFactionsList && this._showDebugControls)
				{
					int index = -1;
					if (!string.IsNullOrEmpty(msgParams[0]))
						index = int.Parse(msgParams[0]);
					if (index >= 0 && index < this._factionsList.Count)
						this.SetSelectedFaction(this._factionsList[index].Name, DesignScreenState.DebugFactionsList);
				}
				else if (panelName == DesignScreenState.DebugEmpireColorList)
				{
					int index = int.Parse(msgParams[0]);
					Vector3 primaryPlayerColor = Player.DefaultPrimaryPlayerColors[index];
					this.App.LocalPlayer.PostSetProp("Color1", (object)new Vector4(primaryPlayerColor.X, primaryPlayerColor.Y, primaryPlayerColor.Z, 1f));
				}
				else if (panelName == "gameCommandList")
				{
				...
				else if (panelName == "gameMissionList")
				{
				...
				else if (panelName == "gameEngineList")
				{
				...
				else if (panelName == DesignScreenState.DebugAnimTracks)
				{
					if (string.IsNullOrEmpty(msgParams[0]))
						this.SetSelectedAnimTrack(string.Empty, DesignScreenState.DebugAnimTracks);
					else
						this.SetSelectedAnimTrack(DesignScreenState._debugAnimTracks[int.Parse(msgParams[0])], DesignScreenState.DebugAnimTracks);
				}
				else if (panelName == DesignScreenState.DebugAnimMode)
				{
					int num = 0;
					if (!string.IsNullOrEmpty(msgParams[0]))
						num = int.Parse(msgParams[0]);
					this._debugCurrentAnimMode = num;
				}
			}
			if (this._selectedDesign.HasValue)
				this.App.UI.SetEnabled("designRemove", true);
			else
				this.App.UI.SetEnabled("designRemove", false);
		}
		...
		private void SetSelectedAnimTrack(string trackName, string trigger)
		{
			...
			if (trigger != DesignScreenState.DebugAnimEdit)
				this.App.UI.SetText(DesignScreenState.DebugAnimEdit, trackName);
			if (!(trigger == DesignScreenState.DebugAnimTracks))
				return;
			int userItemId = -1;
			for (int index = 0; index < DesignScreenState._debugAnimTracks.Length; ++index)
			{
				if (DesignScreenState._debugAnimTracks[index] == trackName)
				{
					userItemId = index;
					break;
				}
			...
			if (userItemId >= 0)
				this.App.UI.SetSelection(DesignScreenState.DebugAnimTracks, userItemId);
			else
				this.App.UI.ClearSelection(DesignScreenState.DebugAnimTracks);
		}
		...
		private void SetSelectedDesign(int designId, string trigger)
		{
			...
			foreach (DesignSectionInfo designSection in designInfo.DesignSections)
			{
				...
				foreach (WeaponBankShipData weaponBank in this._selection.GetCurrentSectionData(sectionType).WeaponBanks)
				{
					...
					weaponBank.Designs.AddRange(this.CollectWeaponDesigns(section1, weaponBank.Bank.TurretClass));
				}
				int num = 0;
				...
				foreach (DesignModuleInfo module in section.Modules)
				{
					...
					{
						DesignScreenState.Warn(string.Format("Module mount {0} not found for design '{1}', section '{2}'.", (object)module.MountNodeName, (object)designInfo.Name, (object)section1.FileName));
					}
					else
					{
		  ...
		  DesignInfo design = null)
		{
			...
			if (trigger != DesignScreenState.UISectionList(sectionType))
				this.App.UI.SetSelection(DesignScreenState.UISectionList(sectionType), sectionTypeShipData.Sections.IndexOf(section1));
			if (section != null)
				ShipDesignUI.SyncSectionArmor(this.App, DesignScreenState.UISectionStats(sectionType), section, design);
		...
		private void SyncWeaponUi()
		{
			...
			if (!DesignScreenState.BankFilters.Any<KeyValuePair<string, DesignScreenState.BankFilter>>((Func<KeyValuePair<string, DesignScreenState.BankFilter>, bool>)(x =>
		   {
			   if (x.Key == "filterAll")
			...
			{
				IEnumerable<WeaponEnums.WeaponSizes> enabledSizes = DesignScreenState.BankFilters.Where<KeyValuePair<string, DesignScreenState.BankFilter>>((Func<KeyValuePair<string, DesignScreenState.BankFilter>, bool>)(x =>
			   {
				   if (x.Value.Value is WeaponEnums.WeaponSizes)
					   return x.Value.Enabled;
				   return false;
			   })).Select<KeyValuePair<string, DesignScreenState.BankFilter>, WeaponEnums.WeaponSizes>((Func<KeyValuePair<string, DesignScreenState.BankFilter>, WeaponEnums.WeaponSizes>)(x => (WeaponEnums.WeaponSizes)x.Value.Value));
		...
		{
			this.App.UI.ClearItems(DesignScreenState.DebugFactionsList);
			for (int userItemId = 0; userItemId < this._factionsList.Count; ++userItemId)
				this.App.UI.AddItem(DesignScreenState.DebugFactionsList, string.Empty, userItemId, this._factionsList[userItemId].Name);
		}
		...
		public void SetSelectedFaction(string factionName, string trigger)
		{
			...
			this.App.LocalPlayer.SetFaction(t.Faction);
			...
			if (trigger != DesignScreenState.DebugFactionsList)
			{
				...
				this.App.UI.SetSelection(DesignScreenState.DebugFactionsList, userItemId);
			}
			if (trigger != "init")
				this.CurrentShipChanged();
		...
		private IEnumerable<RealShipClasses> CollectAvailableShipClasses()
		{
			...
			foreach (RealShipClasses realShipClasses in Enum.GetValues(typeof(RealShipClasses)).Cast<RealShipClasses>())
			{
				RealShipClasses shipClass = realShipClasses;
				if (faction == null || faction.Classes.Any<ClassShipData>((Func<ClassShipData, bool>)(x => x.Class == shipClass)))
		...
		{
			string str = DesignScreenState.UISectionList(sectionType);
			SectionTypeShipData sectionTypeShipData = this._selection.GetCurrentSectionTypeShipData(sectionType);
			...
			if (sectionTypeShipData != null)
			{
				...
				{
					if (this._showDebugControls || !sectionTypeShipData.Sections[userItemId].Section.IsSuulka && !sectionTypeShipData.Sections[userItemId].Section.IsAccelerator && !sectionTypeShipData.Sections[userItemId].Section.IsLoaCube)
		...
		private DesignInfo SummarizeDesign(int playerId, bool finalsummerize = false)
		{
			...
			design.Name = this._RetrofitMode || finalsummerize ? this.App.GameDatabase.ResolveNewDesignName(this.App.LocalPlayer.ID, design.Name) : this.App.GameDatabase.ResolveNewDesignName(this.App.LocalPlayer.ID, string.Empty);
		...
		private void PopulateDesignList()
		{
			if (this._showDebugControls)
				return;
			IEnumerable<DesignInfo> designInfosForPlayer = this.App.GameDatabase.GetVisibleDesignInfosForPlayer(this.App.LocalPlayer.ID, this.SelectedClass);
			List<DesignInfo> list = designInfosForPlayer.ToList<DesignInfo>();
			foreach (DesignInfo design in designInfosForPlayer)
			{
				if (!Kerberos.Sots.StarFleet.StarFleet.IsNewestRetrofit(design, designInfosForPlayer))
					list.Remove(design);
			}
		...
		private void PopulateTechList(string sectionPanel, ShipSectionType sectionType)
		{
			...
			this.App.UI.SetListCleanClear(this.App.UI.Path(sectionPanel, "specialList"), true);
			...
			foreach (string[] shipOption in currentSection.ShipOptions)
			{
				...
				int num = 0;
				foreach (string index in shipOption)
				{
					if (this.App.GameDatabase.PlayerHasTech(this.App.LocalPlayer.ID, index))
					{
						dictionary[index] = false;
						this.App.UI.AddItem(this.App.UI.Path(sectionPanel, "specialList"), string.Empty, userItemId, this.App.AssetDatabase.GetLocalizedTechnologyName(index), "ShipOptionItem");
						...
						this.App.UI.SetTooltip(itemGlobalId, App.Localize("@TECH_DESC_" + index));
						this.App.UI.SetPropertyString(this.App.UI.Path(itemGlobalId, "btnTech"), "id", this.GetTechPanel(sectionType, optionIndex, index));
						++num;
					}
				}
				if (num > 0)
		...
		private void CheckCloakingTech()
		{
			foreach (SectionShipData currentSection in this._selection.GetCurrentSections())
			{
				...
				if (this._shipOptionGroups.ContainsKey(section.Section.Type) && section.Section.cloakingType != CloakingType.None)
				{
					...
					Dictionary<string, bool> source = keyValuePair1.Value.FirstOrDefault<Dictionary<string, bool>>((Func<Dictionary<string, bool>, bool>)(x => x.Any<KeyValuePair<string, bool>>((Func<KeyValuePair<string, bool>, bool>)(y =>
				  {
					  if (!(y.Key == "SLD_Cloaking") && !(y.Key == "SLD_Improved_Cloaking"))
						  return y.Key == "SLD_Intangibility";
					  return true;
				  }))));
					if (source != null && !source.Any<KeyValuePair<string, bool>>((Func<KeyValuePair<string, bool>, bool>)(x => x.Value)))
					{
		...
		private void RefreshCurrentShip()
		{
			...
			DesignInfo design = this.SummarizeDesign(this.App.LocalPlayer.ID, false);
			bool flag3 = this.App.GameDatabase.GetPlayerTechInfos(this.App.LocalPlayer.ID).Any<PlayerTechInfo>((Func<PlayerTechInfo, bool>)(x =>
		   {
			   if (x.State == TechStates.Researched)
				   return x.TechFileID == "ENG_Orbital_Drydocks";
			   return false;
			...
			if (design != null)
			{
				...
				design = this.SummarizeDesign(this.App.LocalPlayer.ID, false);
			}
			if (this._targetArena != null)
			{
			...
			else
			{
				...
				this._builder.New(this.App.LocalPlayer, (IEnumerable<ShipSectionAsset>)((IEnumerable<ShipSectionAsset>)new ShipSectionAsset[3]
				{
		...
		private class CombatShipQueue
		{
			...
			public void AddDesign(App game, ShipBuilder builder, int count, int playerID)
			{
				...
				DesignScreenState.CombatShipQueue.SetText(game, "Added ship " + (object)CombatConfig.GetGameObjectsElement(this._config).ChildNodes.Count);
			}
