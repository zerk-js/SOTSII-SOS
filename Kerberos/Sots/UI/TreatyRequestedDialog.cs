...
// Type: Kerberos.Sots.UI.TreatyRequestedDialog
...
{
	internal class TreatyRequestedDialog : Dialog
	{
		...
		private Dictionary<StationType, string> _stationTypeLocStringMap = new Dictionary<StationType, string>()
	{
	  ...
	  },
	  {
		...
		private const string ConsequencesList = "lstConsequence";
		private const string AcceptButton = "btnAccept";
		private const string DeclineButton = "btnDecline";
		private TreatyInfo _treatyInfo;

		public TreatyRequestedDialog(App game, int treatyId)
		  : base(game, "dialogTreatyRequest")
		{
		...
		public override void Initialize()
		{
			foreach (KeyValuePair<TreatyType, string> treatyPanel in this._treatyPanelMap)
				this._app.UI.SetVisible(treatyPanel.Value, treatyPanel.Key == this._treatyInfo.Type);
			if (this._treatyInfo.Type == TreatyType.Limitation)
				this._app.UI.SetPropertyInt(this.UI.Path(this.ID, "pnlDialogPanel"), "height", 280);
			else
				this._app.UI.SetPropertyInt(this.UI.Path(this.ID, "pnlDialogPanel"), "height", 180);
			...
			switch (this._treatyInfo.Type)
			{
				...
				case TreatyType.Limitation:
					...
					this._app.UI.ClearItems("lstConsequence");
					foreach (TreatyConsequenceInfo consequence in lti.Consequences)
						this._app.UI.AddItem("lstConsequence", string.Empty, consequence.ID, string.Format("{0} {1}", (object)App.Localize(TreatyEditDialog.ConsequenceTypeLocMap[consequence.Type]), (object)consequence.ConsequenceValue));
					string str;
