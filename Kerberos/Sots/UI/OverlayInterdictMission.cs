...
// Type: Kerberos.Sots.UI.OverlayInterdictMission
...
namespace Kerberos.Sots.UI
{
	internal class OverlayInterdictMission : OverlayMission
	{
		...
		{
			Kerberos.Sots.StarFleet.StarFleet.SetInterdictionMission(this.App.Game, this.SelectedFleet, this.TargetSystem, this._useDirectRoute, 0, this.GetDesignsToBuild());
			...
			if (admiralInfo != null)
				this.App.PostRequestSpeech(string.Format("STRAT_005-01_{0}_{1}InterdictMissionConfirmation", (object)this.App.GameDatabase.GetFactionName(this.App.GameDatabase.GetPlayerFactionID(this.App.LocalPlayer.ID)), (object)admiralInfo.GetAdmiralSoundCueContext(this.App.AssetDatabase)), 50, 120, 0.0f);
			this.App.GetGameState<StarMapState>().RefreshMission();
		}

		protected override string GetMissionDetailsTitle()
		{
			return string.Format(App.Localize("@UI_INTERDICT_OVERLAY_MISSION_TITLE"), (object)this.App.GameDatabase.GetStarSystemInfo(this.TargetSystem).Name.ToUpperInvariant());
		}
		...
		protected override void OnRefreshMissionDetails(MissionEstimate estimate)
		{
			...
			this.AddMissionTime(2, App.Localize("@MISSIONWIDGET_INTERDICTION_TIME"), estimate.TurnsAtTarget, hint);
