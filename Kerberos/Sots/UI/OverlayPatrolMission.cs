...
// Type: Kerberos.Sots.UI.OverlayPatrolMission
...
namespace Kerberos.Sots.UI
{
	internal class OverlayPatrolMission : OverlayMission
	{
		...
		{
			Kerberos.Sots.StarFleet.StarFleet.SetPatrolMission(this.App.Game, this.SelectedFleet, this.TargetSystem, this._useDirectRoute, this.GetDesignsToBuild(), this.RebaseTarget);
			...
			if (admiralInfo != null)
				this.App.PostRequestSpeech(string.Format("STRAT_004-01_{0}_{1}PatrolMissionConfirmation", (object)this.App.GameDatabase.GetFactionName(this.App.GameDatabase.GetPlayerFactionID(this.App.LocalPlayer.ID)), (object)admiralInfo.GetAdmiralSoundCueContext(this.App.AssetDatabase)), 50, 120, 0.0f);
			this.App.GetGameState<StarMapState>().RefreshMission();
		}

		protected override string GetMissionDetailsTitle()
		{
			return string.Format(App.Localize("@UI_PATROL_OVERLAY_MISSION_TITLE"), (object)this.App.GameDatabase.GetStarSystemInfo(this.TargetSystem).Name.ToUpperInvariant());
		}
		...
		protected override void OnRefreshMissionDetails(MissionEstimate estimate)
		{
			...
			this.AddMissionTime(2, App.Localize("@MISSIONWIDGET_PATROL_TIME"), estimate.TurnsAtTarget, hint);
