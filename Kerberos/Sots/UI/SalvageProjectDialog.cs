...
// Type: Kerberos.Sots.UI.SalvageProjectDialog
...
namespace Kerberos.Sots.UI
{
	internal class SalvageProjectDialog : Dialog
	{
		...
		public SalvageProjectDialog(App game, string template = "dialogSpecialProjects")
		  : base(game, template)
		{
		}
		...
		private void RefreshSliders()
		{
			foreach (SpecialProjectInfo rate in this._rates)
			{
				...
				int num2 = this._salvagePoints > 0 ? (int)((double)num1 / ((double)rate.Rate * (double)this._salvagePoints)) : 10000;
				if (rate.Progress == -1)
					num2 = this._salvagePoints > 0 ? num1 / this._salvagePoints : 10000;
				++num2;
				if (num2 < 0)
					num2 = 10000;
				if (num2 < 5000)
		...
		private void UpdateResearchSliders(PlayerInfo playerInfo, string iChanged)
		{
			...
			if (iChanged != SalvageProjectDialog.UIGovernmentResearchSlider)
				this._app.UI.SetSliderValue(SalvageProjectDialog.UIGovernmentResearchSlider, (int)num1);
